...
import { DEFAULT_VERSION, EVENT, FONT_FAMILY, WINDOWS_EMOJI_FALLBACK_FONT } from './constants'
...
export const getFontString = ({ fontSize, fontFamily }: { fontSize: number; fontFamily: FontFamilyValues }) => {
 ...
 */
export const arrayToMap = <T extends { id: string } | string>(items: readonly T[] | Map<string, T>) => {
...
export const isShallowEqual = <T extends Record<string, any>, K extends readonly unknown[]>(
  ...
  return aKeys.every((key) => {
    const comparator = (comparators as { [key in keyof T]?: (a: T[key], b: T[key]) => boolean })?.[key as keyof T]
 ...
 */
export const toBrandedType = <BrandedType, CurrentType = BrandedType>(value: Unbrand<BrandedType>) => {
