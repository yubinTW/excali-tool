...
const renderSelectionBorder = (
  ...
  const { angle, elementX1, elementY1, elementX2, elementY2, selectionColors, cx, cy, dashed, activeEmbeddable } =
    elementProperties
      ...
      for (const element of elementsMap.values()) {
        ...
        if (selectionColors.length) {
          ...
          selections.push({
            ...
            cy,
            activeEmbeddable:
              appState.activeEmbeddable?.element === element && appState.activeEmbeddable.state === 'active'
          })
        }
      }
