...
export const isElementInsideBBox = (element: Element, bbox: Bounds, eitherDirection = false): boolean => {
  ...
  const elementInsideBbox =
    bbox[0] <= elementBBox[0] && bbox[2] >= elementBBox[2] && bbox[1] <= elementBBox[1] && bbox[3] >= elementBBox[3]
  ...
  return (
    elementBBox[0] <= bbox[0] && elementBBox[2] >= bbox[2] && elementBBox[1] <= bbox[1] && elementBBox[3] >= bbox[3]
  )
}
...
export const elementPartiallyOverlapsWithOrContainsBBox = (element: Element, bbox: Bounds): boolean => {
  ...
  return (
    (isValueInRange(elementBBox[0], bbox[0], bbox[2]) || isValueInRange(bbox[0], elementBBox[0], elementBBox[2])) &&
    (isValueInRange(elementBBox[1], bbox[1], bbox[3]) || isValueInRange(bbox[1], elementBBox[1], elementBBox[3]))
  )
}
