...
import { Curve, Ellipse, Line, Point, Polycurve, Polygon, Polyline } from './shape'
...
export const pointRotate = (point: Point, angle: number, origin?: Point): Point => {
  const r = angleToRadians(angle)

  if (!origin || isOrigin(origin)) {
    return rotate(point, r)
  }
  return rotate(point.map((c, i) => c - origin[i]) as Point, r).map((c, i) => c + origin[i]) as Point
}

// translate a point by an angle (in degrees) and distance
export const pointTranslate = (point: Point, angle = 0, distance = 0) => {
  const r = angleToRadians(angle)
  return [point[0] + distance * Math.cos(r), point[1] + distance * Math.sin(r)] as Point
}
...
export const lineMidpoint = (line: Line) => {
  return [(line[0][0] + line[1][0]) / 2, (line[0][1] + line[1][1]) / 2] as Point
}
...
export const lineRotate = (line: Line, angle: number, origin?: Point): Line => {
  return line.map((point) => pointRotate(point, angle, origin || lineMidpoint(line))) as Line
}
...
export const curveToBezier = (pointsIn: readonly Point[], curveTightness = 0): Point[] => {
...
const solveCubicEquation = (a: number, b: number, c: number, d: number) => {
  ...
  if (discriminant >= 0) {
    ...
    const root2 = 2 * Math.sqrt(-b / (3 * a)) * Math.cos((Math.acos(realPart) + 2 * Math.PI) / 3)
    const root3 = 2 * Math.sqrt(-b / (3 * a)) * Math.cos((Math.acos(realPart) + 4 * Math.PI) / 3)
...
export const polygonRotate = (polygon: Polygon, angle: number, origin: Point) => {
...
export const polygonScale = (polygon: Polygon, scale: number, origin?: Point) => {
  if (!origin) {
...
export const polygonScaleX = (polygon: Polygon, scale: number, origin?: Point) => {
  if (!origin) {
    origin = polygonCentroid(polygon)
  }
...
export const polygonScaleY = (polygon: Polygon, scale: number, origin?: Point) => {
  if (!origin) {
    origin = polygonCentroid(polygon)
  }
...
export const polygonTranslate = (polygon: Polygon, angle: number, distance: number) => {
...
export const ellipseExtremes = (ellipse: Ellipse) => {
  ...
  const xAtYMax = (yMax * sqSum * sin * cos) / (majorAxis ** 2 * sin ** 2 + minorAxis ** 2 * cos ** 2)

  const xMax = Math.sqrt((sqSum + sqDiff) / 2)
  const yAtXMax = (xMax * sqSum * sin * cos) / (majorAxis ** 2 * cos ** 2 + minorAxis ** 2 * sin ** 2)
...
export const pointRelativeToCenter = (point: Point, center: Point, angle: number): Point => {
...
export const pointOnLine = (point: Point, line: Line, threshold = DEFAULT_THRESHOLD) => {
...
export const pointOnPolyline = (point: Point, polyline: Polyline, threshold = DEFAULT_THRESHOLD) => {
...
export const pointOnCurve = (point: Point, curve: Curve, threshold = DEFAULT_THRESHOLD) => {
  return pointOnPolyline(point, polyLineFromCurve(curve), threshold)
}

export const pointOnPolycurve = (point: Point, polycurve: Polycurve, threshold = DEFAULT_THRESHOLD) => {
...
export const pointOnPolygon = (point: Point, polygon: Polygon, threshold = DEFAULT_THRESHOLD) => {
  let on = false
  const closed = close(polygon)
...
export const polygonIntersectPolygon = (polygonA: Polygon, polygonB: Polygon) => {
  let intersects = false
  let onCount = 0
  const closed = close(polygonA)
...
export const pointOnEllipse = (point: Point, ellipse: Ellipse, threshold = DEFAULT_THRESHOLD) => {
