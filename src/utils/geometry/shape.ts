...
import type { Drawable, Op } from '../../externalLibrary/roughjs/core'
...
// polygon
export const getPolygonShape = (element: RectangularElement): GeometricShape => {
...
// ellipse
export const getEllipseShape = (element: ExcalidrawEllipseElement): GeometricShape => {
...
export const getCurveShape = (
  ...
  const transform = (p: Point) =>
    pointRotate([p[0] + startingPoint[0], p[1] + startingPoint[1]], angleToDegrees(angleInRadian), center)
...
export const getFreedrawShape = (
  ...
  const transform = (p: Point) => pointRotate(pointAdd(p, [element.x, element.y] as Point), angle, center)
...
export const getClosedCurveShape = (
  ...
  const polygonPoints = pointsOnBezierCurves(points, 10, 5).map((p) => transform(p as unknown as Point))
