...
export const shouldEnableBindingForPointerEvent = (
  event: any // React.PointerEvent<HTMLElement>,
) => {
...
const maybeBindBindableElement = (
  ...
  getElligibleElementsForBindableElementAndWhere(bindableElement, app).forEach(([linearElement, where]) =>
    bindOrUnbindLinearElement(
      linearElement,
...
export const updateBoundElements = (
  ...
  getNonDeletedElements(
    ...
    updateBoundPoint(element, 'start', startBinding, changedElement as ExcalidrawBindableElement, elementsMap)
    updateBoundPoint(element, 'end', endBinding, changedElement as ExcalidrawBindableElement, elementsMap)
...
export const getEligibleElementsForBinding = (
  ...
  return selectedElements.flatMap((selectedElement) =>
    isBindingElement(selectedElement, false)
      ? (getElligibleElementsForBindingElement(selectedElement as NonDeleted<ExcalidrawLinearElement>, app).filter(
          (element) => !includedElementIds.has(element.id)
        ) as SuggestedBinding[])
      : isBindableElement(selectedElement, false)
        ? getElligibleElementsForBindableElementAndWhere(selectedElement, app).filter(
...
const getElligibleElementForBindingElement = (
  ...
  return getHoveredElementForBinding(
    getLinearElementEdgeCoors(linearElement, startOrEnd, app.scene.getNonDeletedElementsMap()),
    app
  )
}
...
export const fixBindingsAfterDuplication = (
  ...
  ;(sceneElements.filter(({ id }) => allBoundElementIds.has(id)) as ExcalidrawLinearElement[]).forEach((element) => {
...
export const maxBindingGap = (element: ExcalidrawElement, elementWidth: number, elementHeight: number): number => {
...
const distanceToDiamond = (element: ExcalidrawDiamondElement, point: Point, elementsMap: ElementsMap): number => {
  const [, pointRel, hwidth, hheight] = pointRelativeToElement(element, point, elementsMap)
...
const relativizationToElementCenter = (element: ExcalidrawElement, elementsMap: ElementsMap): GA.Transform => {
  const [x1, y1, x2, y2] = getElementAbsoluteCoords(element, elementsMap)
  const center = coordsCenter(x1, y1, x2, y2)
  // GA has angle orientation opposite to `rotate`
  const rotate = GATransform.rotation(center, element.angle)
...
const getEllipseIntersections = (element: ExcalidrawEllipseElement, gap: number, line: GA.Line): GA.Point[] => {
