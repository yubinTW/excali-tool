...
import { GeometricShape, getPolygonShape, Polygon } from '../utils/geometry/shape'
import { getElementBounds } from './bounds'
import { hasBoundTextElement, isIframeLikeElement, isImageElement, isTextElement } from './typeChecks'
import { ElementsMap, ExcalidrawElement, ExcalidrawRectangleElement } from './types'
...
export const hitElementItself = ({ x, y, element, shape, threshold = 10, frameNameBound = null }: HitTestArgs) => {
  ...
  if (!hit && frameNameBound) {
    hit = isPointInShape([x, y], {
      type: 'polygon',
      data: getPolygonShape(frameNameBound as ExcalidrawRectangleElement).data as Polygon
    })
  }
...
export const hitElementBoundText = (x: number, y: number, textShape: GeometricShape | null) => {
