...
export type SceneBounds = readonly [sceneX: number, sceneY: number, sceneX2: number, sceneY2: number]
 ...
 */
export const getElementLineSegments = (element: ExcalidrawElement, elementsMap: ElementsMap): [Point, Point][] => {
  const [x1, y1, x2, y2, cx, cy] = getElementAbsoluteCoords(element, elementsMap)
...
export const getElementBounds = (element: ExcalidrawElement, elementsMap: ElementsMap): Bounds => {
...
export const getDraggedElementsBounds = (elements: ExcalidrawElement[], dragOffset: { x: number; y: number }) => {
 ...
 */
export const getVisibleSceneBounds = ({ scrollX, scrollY, width, height, zoom }: AppState): SceneBounds => {
