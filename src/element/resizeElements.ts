...
import { adjustXYWithRotation, centerPoint, rotate, rotatePoint } from '../math'
...
import { MaybeTransformHandleType, TransformHandleDirection, TransformHandleType } from './transformHandles'
import {
  isArrowElement,
  isBoundToContainer,
  isFrameLikeElement,
...
const rotateSingleElement = (
  ...
  if (boundTextElementId) {
    const textElement = Scene.getScene(element)?.getElement<ExcalidrawTextElementWithContainer>(boundTextElementId)
...
const getSidesForTransformHandle = (transformHandleType: TransformHandleType, shouldResizeFromCenter: boolean) => {
  return {
...
export const resizeSingleElement = (
  ...
  const newCenter: Point = [newTopLeft[0] + Math.abs(newBoundsWidth) / 2, newTopLeft[1] + Math.abs(newBoundsHeight) / 2]
...
export const resizeMultipleElements = (
  ...
  const scale =
    Math.max(Math.abs(pointerX - anchorX) / (maxX - minX) || 0, Math.abs(pointerY - anchorY) / (maxY - minY) || 0) *
  ...
  for (const { orig, latest } of targetElements) {
    ...
    const boundTextElement = originalElements.get(getBoundTextElementId(orig) ?? '') as
      | ExcalidrawTextElementWithContainer
      | undefined
