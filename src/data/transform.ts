...
const bindLinearElementToElement = (
  ...
  if (end) {
    ...
    if (endType) {
      ...
      bindLinearElement(linearElement, endBoundElement as ExcalidrawBindableElement, 'end', elementsMap)
    }
  }
...
export const convertToExcalidrawElements = (
  ...
  for (const element of elements) {
    ...
    switch (element.type) {
      ...
      case 'ellipse':
      case 'diamond': {
        const width = element?.label?.text && element.width === undefined ? 0 : element?.width || DEFAULT_DIMENSION
        const height = element?.label?.text && element.height === undefined ? 0 : element?.height || DEFAULT_DIMENSION
    ...
    switch (element.type) {
      ...
      case 'arrow': {
        if (element.label?.text) {
          const [container, text] = bindTextToContainer(excalidrawElement, element?.label, elementsMap)
