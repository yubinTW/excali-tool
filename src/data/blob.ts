  ...
  if (blob.type === MIME_TYPES.png) {
    try {
      ...
      if (error.message === 'INVALID') {
        throw new ImageSceneDataError('IMAGE_NOT_CONTAINS_SCENE_DATA')
      } else {
        throw new ImageSceneDataError('Cannot Restore From Image')
      }
    }
  } else {
    if ('text' in Blob) {
    ...
    if (blob.type === MIME_TYPES.svg) {
      try {
        ...
        if (error.message === 'INVALID') {
          ...
          throw new ImageSceneDataError('cannotRestoreFromImage')
        }
      }
    }
  }
...
export const canvasToBlob = async (canvas: HTMLCanvasElement | Promise<HTMLCanvasElement>): Promise<Blob> => {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      canvas.toBlob((blob) => {
        if (!blob) {
          return reject(new CanvasError('CANVAS_POSSIBLY_TOO_BIG'))
        }
        resolve(blob)
      })
    } catch (error: any) {
...
export const generateIdFromDataUrl = (dataURL: string): FileId => {
  const hash = crypto.createHash('sha1').update(dataURL).digest('hex')
...
// TODO: implement by other library which can run on node
...
export const ImageURLToFile = async (imageUrl: string, filename: string = ''): Promise<File | undefined> => {
  ...
  try {
    ...
    throw new Error('failedToFetchImage')
  }
